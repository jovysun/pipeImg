...
function () {
  ...
  _createClass(Dialog, [{
    ...
    value: function renderImgList(imgList, activeIndex) {
      ...
      this._updateActiveData(); // 初始化自定义select


      this.markSelects = _thinSelect__WEBPACK_IMPORTED_MODULE_6__["default"].use(this.$el.find('.J-select-mark')); // 初始化缩略图
      ...
      this._setMarkPosition(this.MARKTYPE.MULTIPLE); // 图片加载完成，激活首屏按钮
    ...
    value: function _bind() {
      ...
      this.$markAllPanel.on('click', '.J-img-thumbnail', function (e) {
        ...
        _this2._updateMarkTxt(_this2.MARKTYPE.MULTIPLE);
...
function loadImages(srcList, success, failure) {
  ...
  var _loadImage = function _loadImage() {
    ...
    var image = new Image();
    var src = srcList[--index]; // IE11对于同域的设置'crossorigin'会导致无法获得image的原始尺寸
